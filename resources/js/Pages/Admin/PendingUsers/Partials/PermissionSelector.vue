...
const toggleGroup = (group) => {
    ...
    if (isGroupAllSelected(group)) {
        props.form.permission_ids = props.form.permission_ids.filter((id) => !ids.includes(id));
    } else {
                ...
                <BCol sm="auto">
                    <div class="form-check form-switch fs-15">
                        <input
                            ...
                            type="checkbox"
                            role="switch"
                            id="toggleAllSellerPermissions"
                            :checked="allSelected"
                            @change="toggleAll"
                        />
                        <label class="form-check-label" for="toggleAllSellerPermissions">
                    ...
                    <div class="border rounded h-100 p-3">
                        ...
                        >
                            <input
                                class="form-check-input"
                                type="checkbox"
                                :id="`seller-perm-${permission.id}`"
                                :checked="isChecked(permission.id)"
                                @change="toggle(permission.id)"
                            />
                            <label
                                class="form-check-label d-flex align-items-center gap-2"
                                :for="`seller-perm-${permission.id}`"
                            >
                                <span>{{ permission.label }}</span>
                                <span v-if="permission.scope" class="badge" :class="badgeClass(permission)">
