...
const billing = computed(() => props.invoice.driver_billing ?? {});
...
const submitPay = () => {
  payForm.post(route("driver-invoices.pay", inv.value.id), {
    forceFormData: true,
    onSuccess: () => {
...
const cancelInvoice = () => {
  Swal.fire({
    title: t("driver_invoices.confirms.cancel_title"),
    text: t("driver_invoices.confirms.cancel_text"),
    icon: "warning",
    ...
    if (result.isConfirmed) {
      router.post(route("driver-invoices.cancel", inv.value.id), {}, { preserveScroll: true });
    }
  });
};
...
const deleteInvoice = () => {
  Swal.fire({
    title: t("driver_invoices.confirms.delete_title"),
    text: t("driver_invoices.confirms.delete_text"),
    icon: "warning",
    showCancelButton: true,
    confirmButtonText: t("driver_invoices.confirms.confirm"),
    cancelButtonText: t("common.cancel"),
    ...
    if (result.isConfirmed) {
      router.delete(route("driver-invoices.destroy", inv.value.id));
    }
  });
};

const openPdf = () => window.open(route("driver-invoices.pdf", inv.value.id), "_blank");
const downloadPdf = () => (window.location = route("driver-invoices.pdf", { driverInvoice: inv.value.id, download: 1 }));
...
<template>
  <Layout>
    <PageHeader :title="$t('driver_invoices.detail.title', { number: inv.invoice_number })" :pageTitle="$t('driver_invoices.detail.page_title')" />
    ...
    <BRow>
      <BCol lg="8">
        ...
        <BCard no-body>
          <BCardHeader class="d-flex align-items-center">
            ...
            <div class="action-bar">
              <BButton variant="soft-secondary" size="sm" :title="$t('driver_invoices.actions.view_pdf')" @click="openPdf">
                <i class="ri-eye-line align-bottom"></i>
                <span class="d-none d-sm-inline ms-1">{{ $t('driver_invoices.actions.view_pdf') }}</span>
              </BButton>
              <BButton variant="soft-primary" size="sm" :title="$t('driver_invoices.actions.download_pdf')" @click="downloadPdf">
                <i class="ri-download-2-line align-bottom"></i>
                <span class="d-none d-sm-inline ms-1">{{ $t('driver_invoices.actions.download_pdf') }}</span>
              </BButton>
            </div>
          </BCardHeader>
          <BCardBody>
        ...
        <BCard no-body>
          <BCardHeader>
            <h5 class="card-title mb-0">{{ $t('driver_invoices.detail.transactions_title') }}</h5>
          </BCardHeader>
          <BCardBody>
        ...
        <BCard no-body>
          <BCardHeader>
            <h5 class="card-title mb-0">{{ $t('driver_invoices.detail.history_title') }}</h5>
          </BCardHeader>
          <BCardBody>
            <div v-if="logs.length === 0" class="text-muted">{{ $t('driver_invoices.detail.no_history') }}</div>
            <ul v-else class="list-unstyled mb-0">
              <li v-for="log in logs" :key="log.id" class="d-flex gap-2 mb-3">
                <i class="ri-history-line text-muted mt-1"></i>
                <div>
                  <div class="fw-medium text-capitalize">{{ log.action.replace(/_/g, ' ') }}</div>
      ...
      <BCol lg="4">
          ...
          <BCardBody>
            <div class="d-grid gap-2">
              ...
              <BButton v-if="can.cancel" variant="soft-danger" @click="cancelInvoice">
                <i class="ri-close-circle-line align-bottom me-1"></i> {{ $t('driver_invoices.actions.cancel') }}
              </BButton>
              <BButton v-if="can.delete" variant="outline-danger" @click="deleteInvoice">
                <i class="ri-delete-bin-line align-bottom me-1"></i> {{ $t('driver_invoices.actions.delete') }}
              </BButton>
              <Link :href="route('driver-invoices.index')" class="btn btn-light">
                <i class="ri-arrow-left-line align-bottom me-1"></i> {{ $t('driver_invoices.actions.back_to_list') }}
              </Link>
            </div>
          </BCardBody>
        </BCard>
        ...
        <BCard no-body>
          <BCardHeader>
            <h5 class="card-title mb-0">{{ $t('driver_invoices.detail.driver_info') }}</h5>
          </BCardHeader>
          <BCardBody>
            <p class="mb-1 fw-semibold">{{ inv.driver?.full_name ?? inv.driver?.name }}</p>
            <p class="text-muted mb-1">{{ inv.driver?.email }}</p>
            <p v-if="billing.phone_number" class="text-muted mb-1">{{ billing.phone_number }}</p>
            <p v-if="billing.cin" class="text-muted mb-1">CIN: {{ billing.cin }}</p>
        ...
        <BCard no-body>
          <BCardHeader>
            <h5 class="card-title mb-0">{{ $t('driver_invoices.detail.payment_info') }}</h5>
          </BCardHeader>
          <BCardBody>
            <div class="d-flex justify-content-between mb-2">
              <span class="text-muted">{{ $t('driver_invoices.detail.bank') }}</span>
              <span>{{ billing.bank_name ?? "—" }}</span>
            </div>
            <div class="d-flex justify-content-between mb-2">
              <span class="text-muted">{{ $t('driver_invoices.detail.rib') }}</span>
              <span>{{ billing.rib ?? "—" }}</span>
            </div>
            <div class="d-flex justify-content-between mb-2">
              <span class="text-muted">{{ $t('driver_invoices.detail.period') }}</span>
              <span>{{ inv.period_start ?? "…" }} → {{ inv.period_end ?? "…" }}</span>
            </div>
            <template v-if="inv.paid_at">
              <hr />
              <div class="d-flex justify-content-between mb-2">
                <span class="text-muted">{{ $t('driver_invoices.detail.paid_by') }}</span>
                <span>{{ inv.paid_by?.full_name ?? inv.paid_by?.name ?? "—" }}</span>
              </div>
              <div class="d-flex justify-content-between mb-2">
                <span class="text-muted">{{ $t('driver_invoices.table.paid_at') }}</span>
              ...
              <a v-if="inv.payment_receipt_url" :href="inv.payment_receipt_url" target="_blank" class="btn btn-soft-secondary btn-sm w-100">
                <i class="ri-attachment-2 align-bottom me-1"></i> {{ $t('driver_invoices.detail.view_receipt') }}
              </a>
            </template>
          </BCardBody>
        </BCard>
