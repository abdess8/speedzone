...
import { PRODUCT_IMPORT_FIELDS, PRODUCT_TEMPLATE_EXAMPLES } from '@/composables/useProductImport';
...
function download() {
  downloadTemplate(
    PRODUCT_IMPORT_FIELDS.map((field) => ({
      header: t(`stock.products.import.fields.${field.key}`),
      example: PRODUCT_TEMPLATE_EXAMPLES[field.key] ?? '',
    })),
    'template_produits.xlsx',
    'Produits'
  );
}
</script>
...
<template>
  <BRow class="g-4">
        ...
        <BCardBody>
          ...
          >
            ...
            <template v-if="parsing">
              ...
              <h5 class="mb-1">{{ $t('stock.products.import.upload.parsing') }}</h5>
            </template>
            ...
            <template v-else>
              ...
              <h5 class="mb-1">{{ $t('stock.products.import.upload.drop_title') }}</h5>
              <p class="text-muted mb-0">{{ $t('stock.products.import.upload.drop_hint') }}</p>
    ...
    <BCol lg="4">
      <BCard no-body class="h-100">
        <BCardHeader>
          <h5 class="card-title mb-0">{{ $t('stock.products.import.upload.template_title') }}</h5>
        </BCardHeader>
        <BCardBody class="d-flex flex-column">
          <p class="text-muted">{{ $t('stock.products.import.upload.template_hint') }}</p>

          <button type="button" class="btn btn-soft-primary w-100 mb-4" @click="download">
            <i class="ri-download-2-line align-bottom me-1"></i>
            {{ $t('stock.products.import.upload.download_template') }}
          </button>

          <h6 class="text-muted text-uppercase fs-12 mb-2">
            {{ $t('stock.products.import.upload.rules_title') }}
          </h6>
          <ul class="text-muted ps-3 mb-0 fs-13 vstack gap-1">
            <li>{{ $t('stock.products.import.upload.rule_header') }}</li>
            <li>{{ $t('stock.products.import.upload.rule_max_rows', { max: maxRows }) }}</li>
            <li>{{ $t('stock.products.import.upload.rule_required') }}</li>
            <li>{{ $t('stock.products.import.upload.rule_sku') }}</li>
            <li>{{ $t('stock.products.import.upload.rule_booleans') }}</li>
          </ul>
        </BCardBody>
      </BCard>
    </BCol>
